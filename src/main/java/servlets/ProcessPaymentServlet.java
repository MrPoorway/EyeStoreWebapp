...
public class ProcessPaymentServlet extends HttpServlet {
    ...
    public void service(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
        ...
        try {
            ...
            for (Cart cart : cartItems) {
                ...
                int availableQty = glasses.getQuantity();
                int qtToBuy = cart.getQuantity();
                availableQty = availableQty - qtToBuy;
                glassesService.updateGlassesQtyById(bCode, availableQty);
                pw.println(this.addGlassesToCard(bCode, bName, bType, bPrice, availableQty));
                session.removeAttribute("qty_" + bCode);
            }
            session.removeAttribute("amountToPay");
            session.removeAttribute("cartItems");
            session.removeAttribute("items");
            session.removeAttribute("selectedGlassesId");
            pw.println("</div>\r\n"
                    + "    </div>");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public String addGlassesToCard(int bCode, String bName, String bType, double bPrice, int bQty) {
        ...
        return "<div class=\"card\">\r\n"
                ...
                + "                        <p class=\"card-text\">\r\n"
                + "                        Type: <span class=\"text-primary\" style=\"font-weight:bold;\"> " + bType
                + "</span><br>\r\n"
                + "                        </p>\r\n"
                + "                        \r\n"
                + "                    </div>\r\n"
